...
def implicit_cf_loss(y_true, y_pred):
    alpha=0.1
    P = y_true > 0
    C = 1 + alpha*y_true
...
# x = keras.layers.Dense(2056, activation='relu')(concat)
...
# model = keras.Model([sender_input, receiver_input], predicted_preference)
model = keras.Model([sender_input, receiver_input], prod)
